...
if (isset($_POST['submit-idin'])) {
    ...
    if(!empty($_FILES["image"]["name"])) { 
        ...
        if(in_array($fileType, $allowTypes)){ 
            ...
            if (mysqli_stmt_prepare($stmt, $sql)) {
                ...
                if($stmt){
                    $success = '<p style=\'color: green; font-size: 20px;\'>Post successfully</p>'; 
                }else{ 
...
}else if (isset($_POST['submit-events'])) {
    ...
    // prepare sql and bind parameters
    $sql = "INSERT INTO events (heading, venue, date, details) 
    VALUES (?, ?, ?, ?)";
    $stmt =mysqli_stmt_init($conn);
    if (mysqli_stmt_prepare($stmt, $sql)) {
        mysqli_stmt_bind_param($stmt, "ssss", $heading, $venue, $date, $details);
        mysqli_stmt_execute($stmt);

        if($stmt){
            $success2 = '<p style=\'color: green; font-size: 20px;\'>Post successfully</p>'; 
        }else{ 
            $success2 = '<p style=\'color: red; font-size: 20px;\'>Not Successful, try again. </p>';
        } 
    }else{ 
        $success2 = '<p style=\'color: red; font-size: 20px;\'>SQL Error. </p>';
    }
}
...
<body>
    ...
    <section id="events" class="call-back red" style="color: #fff;">
        <div class="call-back-heading">
            <h2>Events</h2>
        </div>
        <div class="call-back-container">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <input type="date" name="date" id="" placeholder="Date">
                <textarea class="textarea" name="details" placeholder="Details"></textarea>
                <input type="submit" name="submit-events" value="Post" class="normal blue" style="color: #fff;">
            </form>
        </div>
    </section>
